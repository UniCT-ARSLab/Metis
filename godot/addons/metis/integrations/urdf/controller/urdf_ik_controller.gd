...
## Jacobian-based IK controller for a URDF arm.
## It emits bounded joint velocities in the normalized action space used by RL agents.
...
## `pointing` leaves roll free; `full` matches the target basis.
@export_enum("pointing", "full") var orientation_mode := "pointing"
## End-effector axis used by pointing mode.
...
## Computes one normalized IK action and optionally applies it to the robot.
func solve(apply: bool = true) -> PackedFloat32Array:
	...
	# Build geometric Jacobian columns in the moving child-link frame.
	...
	# Position uses three rows; full pose adds three orientation rows.
	...
	# Damped least squares: (J Jt + lambda^2 I)x = e, then dq = Jt x.
...
## Solves a small dense system with partial-pivot Gaussian elimination.
...
## Returns the shortest rotation that aligns the approach axes without changing roll.
