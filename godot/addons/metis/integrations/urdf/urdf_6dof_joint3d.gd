...
func set_motor_target_velocity(target_velocity: float) -> void:
	if joint and joint.type == "prismatic":
		set_flag_z(Generic6DOFJoint3D.FLAG_ENABLE_LINEAR_MOTOR, true)
		set_param_z(
...
func set_spring_target_position(
	...
	if joint and joint.type == "prismatic":
		set_flag_z(Generic6DOFJoint3D.FLAG_ENABLE_LINEAR_SPRING, true)
		set_param_z(
...
func update_joint(
	...
	elif joint.type == "prismatic" and joint.limit:
		var lower := minf(joint.limit.lower, joint.limit.upper)
		var upper := maxf(joint.limit.lower, joint.limit.upper)
		self.set_param_z(
			Generic6DOFJoint3D.PARAM_LINEAR_LOWER_LIMIT, lower)
		self.set_param_z(
			Generic6DOFJoint3D.PARAM_LINEAR_UPPER_LIMIT, upper)

	var is_movable := joint.type in ["revolute", "continuous", "prismatic"]
	var is_prismatic := joint.type == "prismatic"
	self.set_flag_z(
		Generic6DOFJoint3D.FLAG_ENABLE_LINEAR_MOTOR,
		is_movable and is_prismatic)
	self.set_flag_z(
		Generic6DOFJoint3D.FLAG_ENABLE_MOTOR,
		is_movable and not is_prismatic)
	if is_prismatic:
		self.set_param_z(
	...
	if joint.dynamics:
		...
		if joint.dynamics.damping > 0:
			...
			if is_prismatic:
				self.set_flag_z(
					Generic6DOFJoint3D.FLAG_ENABLE_LINEAR_SPRING, true)
				self.set_param_z(
					Generic6DOFJoint3D.PARAM_LINEAR_SPRING_DAMPING,
					joint.dynamics.damping)
			else:
	...
	if joint.limit and joint.limit.effort > 0:
		...
		self.set_param_z(
			(
				Generic6DOFJoint3D.PARAM_LINEAR_MOTOR_FORCE_LIMIT
				if is_prismatic
				else Generic6DOFJoint3D.PARAM_ANGULAR_MOTOR_FORCE_LIMIT
			),
			max_force)
