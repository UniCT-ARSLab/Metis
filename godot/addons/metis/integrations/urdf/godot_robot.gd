...
func set_joint_target_velocity(joint_name: String, target_velocity: float) -> bool:
	...
	if joint_data.is_mimic():
		...
		return false
	if joint_data.type not in ["revolute", "continuous", "prismatic"]:
...
func _sync_mimic_targets() -> void:
	...
	for mimic in _mimic_order:
		...
		var source_is_prismatic := (
			source.joint and source.joint.type == "prismatic")
		var follower_is_prismatic := (
			follower.joint and follower.joint.type == "prismatic")
		var source_velocity := source.get_param_z(
			(
				Generic6DOFJoint3D.PARAM_LINEAR_MOTOR_TARGET_VELOCITY
				if source_is_prismatic
				else Generic6DOFJoint3D.PARAM_ANGULAR_MOTOR_TARGET_VELOCITY
			))
		...
		var spring_enabled := source.get_flag_z(
			(
				Generic6DOFJoint3D.FLAG_ENABLE_LINEAR_SPRING
				if source_is_prismatic
				else Generic6DOFJoint3D.FLAG_ENABLE_ANGULAR_SPRING
			))
		follower.set_flag_z(
			(
				Generic6DOFJoint3D.FLAG_ENABLE_LINEAR_SPRING
				if follower_is_prismatic
				else Generic6DOFJoint3D.FLAG_ENABLE_ANGULAR_SPRING
			),
			spring_enabled)
		if spring_enabled:
			var source_target := source.get_param_z(
				(
					...
					if source_is_prismatic
					else Generic6DOFJoint3D.PARAM_ANGULAR_SPRING_EQUILIBRIUM_POINT
				))
			follower.set_spring_target_position(
				...
				source.get_param_z(
					(
						...
						if source_is_prismatic
						else Generic6DOFJoint3D.PARAM_ANGULAR_SPRING_STIFFNESS
					)),
					source.get_param_z(
						(
							Generic6DOFJoint3D.PARAM_LINEAR_SPRING_DAMPING
							if source_is_prismatic
							else Generic6DOFJoint3D.PARAM_ANGULAR_SPRING_DAMPING
						)))
...
func _get_kinematic_link_transform(
	...
	elif joint.type == "prismatic" and not joint.axis.is_zero_approx():
		motion.origin = joint.axis.normalized() * get_joint_position(joint.name)
